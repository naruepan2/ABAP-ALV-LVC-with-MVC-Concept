CLASS zcl_mvcfw_base_model DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS get_component_of_data
      ...
      RETURNING
        VALUE(rt_component) TYPE abap_compdescr_tab .
  ...
  PRIVATE SECTION.
    ...
    METHODS _get_incl_components
      IMPORTING
        !io_type      TYPE REF TO cl_abap_datadescr
      CHANGING
        !ct_component TYPE abap_compdescr_tab .
...
*      lo_out = REF #( ct_data ).
*    ELSE.
*      lo_out = me->get_outtab( ).
*    ENDIF.
*    IF lo_out IS BOUND.
*      ASSIGN lo_out->* TO <lft_out>.
*    ENDIF.
*
*    DATA(lt_comp) = get_component_of_data( EXPORTING it_data = <lft_out> ).
*
*
*    LOOP AT <lft_out> ASSIGNING <lfs_out>.
*      LOOP AT lt_comp INTO DATA(ls_comp).
*        ASSIGN COMPONENT ls_comp-name OF STRUCTURE <lfs_out> TO FIELD-SYMBOL(<lf_val>).
*        CHECK sy-subrc EQ 0.
*
...
**           iv_folder       =
*            io_tree         = io_view->mo_tree
*          IMPORTING
*            ev_last_key     = DATA(lv_last_key)
*          RECEIVING
*            ro_node         = DATA(lo_node).
*
**      "Set data setting
**      TRY.
  ...
  METHOD get_component_of_data.
    ...
    LOOP AT lt_comp INTO DATA(ls_comp).
      IF ls_comp-name IS NOT INITIAL.
        DATA(lr_type) = CAST cl_abap_datadescr( ls_comp-type ).
        rt_component  = VALUE #( BASE rt_component
                                 ( length    = lr_type->length
                                   decimals  = lr_type->decimals
                                   type_kind = lr_type->type_kind
                                   name      = ls_comp-name ) ).
      ELSEIF ls_comp-as_include IS NOT INITIAL.
        _get_incl_components( EXPORTING io_type      = ls_comp-type
                              CHANGING  ct_component = rt_component ).
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
    ...
    IF ls_scolo IS NOT INITIAL.
      ...
      LOOP AT lt_comp INTO DATA(ls_comp).
        READ TABLE ct_color ASSIGNING <lfs_color> BINARY SEARCH
          WITH KEY fname = ls_comp-name.
        IF sy-subrc EQ 0.
          <lfs_color>-fname    = ls_comp-name.
          <lfs_color>-color    = ls_scolo.
          <lfs_color>-nokeycol = iv_nokeycol.
        ELSE.
          CLEAR ls_color.
          ls_color-fname    = ls_comp-name.
...
*   convert to uppercase if necessary
  ...
  METHOD _get_incl_components.
    DATA lo_stru TYPE REF TO cl_abap_structdescr.
    DATA lt_comp TYPE STANDARD TABLE OF abap_componentdescr.

    CHECK io_type IS BOUND.

    lo_stru = CAST cl_abap_structdescr( io_type ).
    CHECK lo_stru IS BOUND.
    ...
    LOOP AT lt_comp INTO DATA(ls_comp).
      IF ls_comp-name IS NOT INITIAL.
        DATA(lr_type) = CAST cl_abap_datadescr( ls_comp-type ).
        ct_component  = VALUE #( BASE ct_component
                                 ( length    = lr_type->length
                                   decimals  = lr_type->decimals
                                   type_kind = lr_type->type_kind
                                   name      = ls_comp-name ) ).
      ELSEIF ls_comp-as_include IS NOT INITIAL.
        _get_incl_components( EXPORTING io_type      = ls_comp-type
                              CHANGING  ct_component = ct_component ).
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
