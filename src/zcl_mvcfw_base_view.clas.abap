class ZCL_MVCFW_BASE_VIEW definition
  public
  create public .
...
public section.
  ...
  methods USER_COMMAND
    importing
      ...
      !IO_CNTLR type ref to ZCL_MVCFW_BASE_CONTROLLER optional
  ...
  methods REGISTER_EVENT
    ...
    returning
      value(RO_VIEW) type ref to ZCL_MVCFW_BASE_VIEW .
  ...
  methods CREATE_NEW_VIEW_TO_CONTROLLER
    importing
      ...
      !IO_MODEL type ref to ZCL_MVCFW_BASE_MODEL optional
      !IO_CNTLR type ref to ZCL_MVCFW_BASE_CONTROLLER optional
      !IR_ACTION type ref to ZCL_MVCFW_BASE_CONTROLLER=>TY_VIEW_ACTION optional
    returning
      value(RO_VIEW) type ref to ZCL_MVCFW_BASE_VIEW
    raising
      ZBCX_EXCEPTION .
  ...
  methods ADD_SALV_TREE_NODE
    ...
    returning
      value(RO_NODE) type ref to CL_SALV_NODE .
  methods MODIFY_SALV_COLUMNS
    importing
      !IT_COLUMN type SALV_T_COLUMN_REF .
  methods MODIFY_SALV_SETTING
    importing
      !IO_SETTING type ref to CL_SALV_TREE_SETTINGS .
  methods MODIFY_SALV_FUNCTIONS
    importing
      !IO_FUNCTION type ref to CL_SALV_FUNCTIONS_TREE .
  methods MODIFY_SALV_EVENTS
    importing
      !IO_EVENTS type ref to CL_SALV_EVENTS_TREE .
  methods SET_SALV_COL_TEXT
    importing
      !IO_COLUMN type ref to CL_SALV_COLUMN
      !IV_TEXT type ANY optional
      !IV_TEXT_S type SCRTEXT_S optional
      !IV_TEXT_M type SCRTEXT_M optional
      !IV_TEXT_L type SCRTEXT_L optional .
  methods SET_SALV_COL_TECHNICAL
    importing
      !IO_COLUMN type ref to CL_SALV_COLUMN
      !IV_VALUE type SAP_BOOL default IF_SALV_C_BOOL_SAP=>TRUE .
  methods SET_SALV_COL_VISIBLE
    importing
      !IO_COLUMN type ref to CL_SALV_COLUMN
      !IV_VALUE type SAP_BOOL default IF_SALV_C_BOOL_SAP=>TRUE .
  ...
  methods ON_TREE_DOUBLE_CLICK
    ...
    importing
      !NODE_KEY
      !COLUMNNAME .
  methods ON_TREE_KEYPRESS
    for event KEYPRESS of CL_SALV_EVENTS_TREE
    importing
      !NODE_KEY
      !COLUMNNAME
      !KEY .
  methods ON_TREE_CHECKBOX_CHANGE
    for event CHECKBOX_CHANGE of CL_SALV_EVENTS_TREE
    importing
      !COLUMNNAME
      !NODE_KEY
      !CHECKED .
  methods ON_TREE_EXPAND_EMPTY_FOLDER
  ...
  PROTECTED SECTION.
    ...
    DATA lmo_cntlr TYPE REF TO zcl_mvcfw_base_controller .
  ...
  PRIVATE SECTION.
    ...
    METHODS _get_salv_tree_columns
      RETURNING
        VALUE(rt_columns) TYPE salv_t_column_ref .
    METHODS _get_salv_tree_setting
      RETURNING
        VALUE(ro_setting) TYPE REF TO cl_salv_tree_settings .
    METHODS _get_salv_tree_functions
      RETURNING
        VALUE(ro_functions) TYPE REF TO cl_salv_functions_tree .
    METHODS _get_salv_tree_events
      RETURNING
        VALUE(ro_events) TYPE REF TO cl_salv_events_tree .
...
CLASS ZCL_MVCFW_BASE_VIEW IMPLEMENTATION.


  METHOD add_salv_tree_node.
    DATA: lo_nodes TYPE REF TO cl_salv_nodes.

    lo_nodes = io_tree->get_nodes( ).

    TRY.
        ro_node = lo_nodes->add_node( related_node    = iv_related_node
...
*    ENDLOOP.
  ...
  METHOD create_new_view_to_controller.
    IF iv_stack_name IS INITIAL.
      RAISE EXCEPTION TYPE zbcx_exception
        EXPORTING
          iv_msgv1 = 'Please enter stack name'.
    ENDIF.
    ...
    TRY.
        zcl_mvcfw_base_controller=>set_stack_name( iv_stack_name = |{ iv_stack_name CASE = UPPER }|
                                                   io_model      = io_model
                                                   io_view       = me ).
        zcl_mvcfw_base_controller=>set_view_action( ir_action ).
  ...
  METHOD destroy_view.
    ...
    zcl_mvcfw_base_controller=>destroy_stack( EXPORTING iv_name         = iv_name
  ...
  METHOD display_salv_tree.

    IF ct_data IS SUPPLIED OR iv_create_tree_directly IS NOT INITIAL.
      mo_tree = _create_salv_tree_object( EXPORTING iv_create_tree_directly = iv_create_tree_directly
                                          CHANGING  ct_data                 = ct_data ).
    ELSE.
      mo_tree = _create_salv_tree_object( ).
    ENDIF.
    IF mo_tree IS NOT BOUND.
      RAISE EXCEPTION TYPE zbcx_exception
        EXPORTING
          iv_msgv1 = 'Error while tree was created'.
    ENDIF.
    ...
    modify_salv_columns( _get_salv_tree_columns( ) ).
    modify_salv_setting( _get_salv_tree_setting( ) ).
    modify_salv_functions( _get_salv_tree_functions( ) ).
    modify_salv_events( _get_salv_tree_events( ) ).
  ...
  METHOD display_sub_alv.
    IF lmo_cntlr IS NOT BOUND.
      RAISE EXCEPTION TYPE zbcx_exception
        EXPORTING
          iv_msgv1 = 'Controller was not created'.
    ENDIF.
  ...
  METHOD export_view_data_to_xls.
    ...
    TRY.
        ...
        IF table IS NOT BOUND.
          RAISE EXCEPTION TYPE zbcx_exception
            EXPORTING
              iv_msgv1 = 'Cannot create SALV table'.
        ENDIF.
        ...
        lr_columns->get_column( 'ALV_CELL' )->set_technical( ).
        lr_columns->get_column( 'ALV_COLOR' )->set_technical( ).
      CATCH cx_salv_msg
            cx_salv_not_found.
    ENDTRY.
    ...
    IF iv_filename IS INITIAL.
      ...
      TRY.
        ...
        CATCH cx_sy_itab_line_not_found.
          RAISE EXCEPTION TYPE zbcx_exception
            EXPORTING
              iv_msgv1 = 'File name is required'.
      ENDTRY.
    ENDIF.
...
*--------------------------------------------------------------------*
  ...
  METHOD modify_salv_columns.
*    LOOP AT it_column INTO DATA(column).
...
*    ENDLOOP.
  ...
  METHOD modify_salv_events.
*    SET HANDLER link_click
*                double_click
*                keypress
*                checkbox_change
*                expand_empty_folder FOR io_events.
  ENDMETHOD.


  METHOD modify_salv_functions.
  ENDMETHOD.


  METHOD modify_salv_setting.
  ENDMETHOD.
...
*      _prepare_default_alv( IMPORTING et_event = et_event ).
  ...
  METHOD set_default_grid_handler.
    DATA: lo_events TYPE REF TO zcl_mvcfw_base_view.
  ...
  METHOD set_salv_col_technical.
    io_column->set_technical( iv_value ).
  ENDMETHOD.


  METHOD set_salv_col_text.
    IF iv_text IS NOT INITIAL.
      io_column->set_short_text( iv_text ).
      io_column->set_medium_text( iv_text ).
      io_column->set_long_text( iv_text ).
    ELSE.
      io_column->set_short_text( iv_text_s ).
      io_column->set_medium_text( iv_text_m ).
      io_column->set_long_text( iv_text_l ).
    ENDIF.
  ENDMETHOD.


  METHOD set_salv_col_visible.
    io_column->set_visible( iv_value ).
  ENDMETHOD.
  ...
  METHOD _auto_gen_stack_name.
    ...
    DATA(lt_stack) = zcl_mvcfw_base_controller=>get_all_stack( ).
  ...
  METHOD _check_grid_oops_toolbar.
    DATA: lo_event_receiver TYPE REF TO zcl_mvcfw_base_view.
  ...
  METHOD _check_routine.
    ...
    TRY.
        ...
        zcl_mvcfw_base_controller=>check_routine( EXPORTING iv_set_value = abap_true ).
        PERFORM (lv_formname) IN PROGRAM (lmv_repid).
      CATCH cx_sy_dyn_call_illegal_form.
        ev_found = abap_false.
        zcl_mvcfw_base_controller=>check_routine( EXPORTING iv_set_value = abap_false ).
        RETURN.
  ...
  METHOD _get_salv_tree_columns.
    mo_tree->get_columns( )->set_optimize( abap_true ).
    rt_columns = mo_tree->get_columns( )->get( ).
  ENDMETHOD.


  METHOD _get_salv_tree_events.
    ro_events = mo_tree->get_event( ).
  ENDMETHOD.


  METHOD _get_salv_tree_functions.
    ro_functions = mo_tree->get_functions( ).
    ro_functions->set_all( ).
  ENDMETHOD.


  METHOD _get_salv_tree_setting.
    ro_setting = mo_tree->get_tree_settings( ).
    ro_setting->set_hierarchy_header( 'Hierarchical ALV Tree').
    ro_setting->set_hierarchy_size( 40 ).
    ro_setting->set_hierarchy_icon( CONV #( icon_tree ) ).
  ENDMETHOD.
