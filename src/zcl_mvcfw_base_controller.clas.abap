CLASS zcl_mvcfw_base_controller DEFINITION
  ...
  PUBLIC SECTION.
    ...
    DATA mo_view TYPE REF TO zcl_mvcfw_base_lvc_view .
...
private section.
  ...
  class-methods _CLEAR_VIEW_ACTION .
ENDCLASS.
  ...
  METHOD handle_check_changed_data.
    TRY.
        ...
        IF lo_stack IS BOUND AND lo_stack->view IS BOUND.
          lo_stack->view->check_changed_data( io_data_changed = io_data_changed
                                              io_model        = lmo_current_model
                                              iv_stack_name   = lo_stack->name ).
        ELSEIF mo_view IS BOUND.
          mo_view->check_changed_data( io_data_changed = io_data_changed
                                       io_model        = lmo_current_model
                                       iv_stack_name   = lmv_current_stack ).
        ELSE.
          DATA(lo_view) = NEW zcl_mvcfw_base_lvc_view( ).
          IF lo_view IS BOUND.
            lo_view->check_changed_data( io_data_changed = io_data_changed
                                         io_model        = lmo_current_model
                                         iv_stack_name   = lmv_current_stack ).
          ENDIF.
        ENDIF.
